...
def create_session(username: str = None, password: str = None, domain: str = "GUC") -> requests.Session:
    ...
    if username and password:
        if HttpNtlmAuth:
            ...
            logger.error("NTLM auth requested but requests_ntlm library is not installed.")
    ...
    session.timeout = (config.DEFAULT_REQUEST_TIMEOUT, config.DEFAULT_REQUEST_TIMEOUT * 2)
...
def make_request(session: requests.Session, url: str, method: str = "GET", **kwargs) -> requests.Response | None:
    """
    ...
    try:
        ...
        if response.history: # Check if redirection occurred
             for resp_hist in response.history:
                  if 'login' in resp_hist.url.lower():
                       logger.warning(f"Request redirected to login page during history for {method} {url}")
                       return None # Treat login redirect as failure
        if 'login' in response.url.lower() and response.status_code != 401: # Check final URL
             ...
             if temp_soup.find('form', action=lambda x: x and 'login' in x.lower()):
                 logger.warning(f"Request landed on login page (form detected) for {method} {url}")
        ...
        logger.error(f"HTTP error for {method} {url}: {e.response.status_code} {e.response.reason}")
        return None
        ...
        return None
         ...
         logger.error(f"Unexpected error during request for {method} {url}: {e}", exc_info=True)
         return None