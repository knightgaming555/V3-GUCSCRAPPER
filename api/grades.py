...
def api_grades():
    ...
    try:
        ...
        if grades_data is None:
            ...
            return jsonify({"status": "error", "message": "Failed to fetch grades data due to a server error"}), 500
            ...
            logger.error(f"Grades scraping returned specific error for {username}: {error_msg}")
            ...
            set_in_cache(cache_key, grades_data, timeout=config.CACHE_DEFAULT_TIMEOUT) # Use default cache timeout
        ...
        logger.warning(f"AuthError during grades request for {username}: {e.log_message}")
