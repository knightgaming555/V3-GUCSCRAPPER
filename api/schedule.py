...
from scraping.schedule import scrape_schedule, filter_schedule_details # Import necessary functions
    ...
    try:
        ...
        if cached_data:
            ...
            if isinstance(cached_data, (list, tuple)) and len(cached_data) == 2:
                ...
                return jsonify(cached_data), 200
                 ...
                 logger.warning(f"Invalid schedule data format found in cache for {username}. Fetching fresh data.")
        ...
        if isinstance(raw_schedule_data, dict) and "error" in raw_schedule_data:
            ...
            if "Authentication failed" in error_msg:
                ...
                status_code = 401
            elif any(e in error_msg for e in ["timeout", "Connection error", "HTTP error", "Failed to fetch"]):
                ...
                status_code = 504 # Gateway timeout / upstream error
                 ...
                 status_code = 500
            return jsonify({"status": "error", "message": error_msg, "data": None}), status_code
            ...
            logger.error(f"Schedule scraping returned invalid data type for {username}: {type(raw_schedule_data)}")
            g.log_outcome = "scrape_no_result"
            g.log_error_message = "Scraping function returned invalid data"
            return jsonify({"status": "error", "message": "Failed to fetch schedule data (invalid result)"}), 500
        else:
            # --- Success ---
            ...
            set_in_cache(cache_key, response_data, timeout=config.CACHE_LONG_TIMEOUT) # Use long timeout
        ...
        logger.warning(f"AuthError during schedule request for {username}: {e.log_message}")
        g.log_outcome = e.log_outcome
        g.log_error_message = e.log_message
        return jsonify({"status": "error", "message": str(e)}), e.status_code
    except Exception as e:
        # Catch unexpected errors in the endpoint logic
        logger.exception(f"Unhandled exception during /api/schedule request for {username}: {e}")
